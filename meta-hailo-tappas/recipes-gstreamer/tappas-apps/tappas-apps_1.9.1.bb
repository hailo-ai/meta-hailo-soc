...
S = "${WORKDIR}/git/apps/h15/native/internals"
...
do_install:append() {
    if [ '${IS_H15}' = 'true' ]; then
        ...
        # Try multiple possible source paths for the JSON files
        if [ -d "${S}/encoder_pipelines_new_api" ]; then
            install -m 0755 ${S}/encoder_pipelines_new_api/*.json ${ROOTFS_APPS_DIR}/encoder_pipelines_new_api/configs/
        elif [ -d "${WORKDIR}/git/apps/h15/native/internals/encoder_pipelines_new_api" ]; then
            install -m 0755 ${WORKDIR}/git/apps/h15/native/internals/encoder_pipelines_new_api/*.json ${ROOTFS_APPS_DIR}/encoder_pipelines_new_api/configs/
        else
            bbwarn "encoder_pipelines_new_api directory not found in ${S} or ${WORKDIR}/git/apps/h15/native/internals/"
        fi
        install -m 0755 ${HAILO15_DIR}/manage_config_tuning.sh ${ROOTFS_APPS_DIR}/manage_config_tuning.sh
    fi
}
